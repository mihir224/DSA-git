...
class Heaps {
    ...
    //brute (gives TLE) - find all possible combinations and insert them in a min heap such that at max heap could carry only C elements.
    ...
    public ArrayList<Integer> solve(ArrayList<Integer> A, ArrayList<Integer> B, int C) {
        ...
        set.add(new Pair3(n-1,m-1));
        while(C>0){
            Triad t=pairSum.poll();
            ans.add(t.first);
            int l=t.second;
            int r=t.third;
            if(l>0&&r>0&&!set.contains(new Pair3(l-1,r))){
                pairSum.offer(new Triad(A.get(l-1)+B.get(r),l-1,r));
                set.add(new Pair3(l-1,r));
            }
            if(l>0&&r>0&&!set.contains(new Pair3(l,r-1))){
                pairSum.offer(new Triad(A.get(l)+B.get(r-1),l,r-1));
                set.add(new Pair3(l,r-1));
            }
            C--;
        }
        return ans;
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for(int key:map.keySet()){
            list.add(new Pair4(map.get(key),key));
        }
    ...
    class Pair3 implements Comparable<Pair4>{
        int first;
        int second;
    ...
    public int[] topKFrequent1(int[] nums, int k) {
        ...
        for(int key:map.keySet()){
            pq.offer(new Pair4(map.get(key),key));
